...
int main(int argc,char **argv) {
    ...
    for (unsigned i=0;dirs[i];i++) {
        ...
        if (!strcmp(dirs[i],"dev") || !strcmp(dirs[i],"sys") || !strcmp(dirs[i],"tmp")) flags|=MS_REC;
    ...
    execl("/jffs/docker/bin/dockerd","dockerd","--config-file=/jffs/docker/config/daemon.json",(char *)NULL);
    die("exec dockerd");
}
