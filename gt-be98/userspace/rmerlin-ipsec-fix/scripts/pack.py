...
p = argparse.ArgumentParser(); p.add_argument('--revision', default='ipsec-fix-v1'); args = p.parse_args()
...
image = r / 'candidate/GT-BE98_leon36-rmerlin1029a2_zstd22.pkgtb'
...
(r / 'evidence/packaging.json').write_text(json.dumps({
    ...
    'compression': 'zstd level 22, 1 MiB blocks, tailends, type/size ordering', 'flashed': False,
}, indent=2) + '\n')
print('RMERLIN_CANDIDATE_PACKED', sha(image))
