...
archive=r/'leon8-physical-trial.tar.zst';assert not archive.exists()
paths=['README.md','result.json','scripts','flash/scripts','flash/evidence',
       'flash/expected-files.json','flash/required-checks.json','flash/leon8-probes.tar','candidate/'+next((r/'candidate').glob('*.manifest.json')).name]
...
files=[]
for name in ['README.md','result.json','physical-backup-manifest.json','flash/scripts','flash/evidence','flash/expected-files.json','flash/required-checks.json',
...
with tarfile.open(r/'physical-publication.tar','w') as tar:
    for p in sorted(set(files)):
        ...
        name='gt-be98/userspace/rc-ssh/'+p.relative_to(r).as_posix()
