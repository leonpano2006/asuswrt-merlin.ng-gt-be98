...
work=$(mktemp -d /run/leon8-features-XXXXXX)
unit=leon8-journal-probe.service
server=''
cleanup() {
...
printf 'leon8 ephemeral credential test\n' >"$work/plain"
...
while test "$i" -lt 300; do
    printf 'LEON8-ZSTD-%04d abcdefghijklmnopqrstuvwxyz-repeated-journal-data ' "$i"
    i=$((i+1))
done
echo
EOF
...
grep -q LEON8-ZSTD-0299 "$work/message-read"
...
server=$!
sleep 1
/tmp/leon8-probes/curl-native-test --noproxy '*' --fail --silent --show-error --max-time 15 --cacert "$work/tls.crt" --resolve localhost:24443:127.0.0.1 https://localhost:24443/ >"$work/tls-result"
wait "$server";server=''
grep -q s_server "$work/tls-result"
echo LIVE_NATIVE_CURL_OPENSSL4_TLS13_PASS
/usr/bin/bash /tmp/leon8-probes/shell-coreutils-check.bash
...
touch /run/leon8-live-features-passed
echo LIVE_LEON8_FEATURES_ALL_PASS
