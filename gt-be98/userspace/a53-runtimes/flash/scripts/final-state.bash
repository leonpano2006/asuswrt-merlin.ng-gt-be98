...
/usr/local/bin/docker ps -a --filter label=leon.test=a53-runtimes
/usr/local/bin/docker network ls --filter label=leon.test=a53-runtimes
test -z "$(/usr/local/bin/docker ps -aq --filter label=leon.test=a53-runtimes)"
test -z "$(/usr/local/bin/docker network ls -q --filter label=leon.test=a53-runtimes)"
...
echo A53_RUNTIME_FINAL_UNCOMMITTED_STATE_PASS
