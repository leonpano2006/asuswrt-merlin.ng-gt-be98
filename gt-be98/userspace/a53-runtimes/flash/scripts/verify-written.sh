...
test "$(cat /sys/class/ubi/ubi0_4/reserved_ebs)" = 615
boot=$(sha256sum /dev/ubi0_3 | cut -d ' ' -f 1)
root=$(head -c 77021184 /dev/ubi0_4 | sha256sum | cut -d ' ' -f 1)
...
test "$root" = 68888e499d0095374266c8f2adf0be296c5370e893c81c548e7a36c1030e9216
...
test "$(cat /sys/class/ubi/ubi0/avail_eraseblocks)" = 12
