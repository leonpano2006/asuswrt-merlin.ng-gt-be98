...
test "$("$docker" inspect "$server" --format '{{index .Config.Labels "leon.test"}}')" = a53-runtimes
test "$("$docker" network inspect "$network" --format '{{index .Labels "leon.test"}}')" = a53-runtimes
...
test -z "$("$docker" ps -aq --filter label=leon.test=a53-runtimes)"
test -z "$("$docker" network ls -q --filter label=leon.test=a53-runtimes)"
echo A53_MEMCG_AND_TEST_CLEANUP_PASS
