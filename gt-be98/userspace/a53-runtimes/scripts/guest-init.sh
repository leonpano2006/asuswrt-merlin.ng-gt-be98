...
test "$failed" = 0
# Exercise libgcc unwind ABI, arithmetic, forced cancellation and C++ DSOs.
mkdir -p /newroot/tmp/libgcc-tests
for abi in aarch64 armel armhf armel-legacy aarch64-legacy; do
    ...
    for name in runtime-$abi exception-$abi throw-$abi.so; do
        cp /$name /newroot/tmp/libgcc-tests/$name
    done
    echo "LAB_LIBGCC_BEGIN $abi"
    EXPECTED_TRIPLET="$triplet" chroot /newroot /tmp/libgcc-tests/runtime-$abi
    chroot /newroot /tmp/libgcc-tests/exception-$abi /tmp/libgcc-tests/throw-$abi.so
    for stringabi in 0 1; do
        name=cxx-$abi-abi$stringabi
        cp /$name /newroot/tmp/libgcc-tests/$name
        EXPECTED_TRIPLET="$triplet" chroot /newroot /tmp/libgcc-tests/$name
    done
    echo "LAB_LIBGCC_COMPLETE $abi"
done
