...
p.add_argument('--library-probe', type=Path, required=True)
p.add_argument('--libgcc-probes', type=Path, required=True)
a = p.parse_args()
...
for abi in ('aarch64', 'armel', 'armhf', 'armel-legacy', 'aarch64-legacy'):
    for stem, suffix in [('runtime-', ''), ('exception-', ''), ('throw-', '.so')]:
        name = stem + abi + suffix
        files[name] = a.libgcc_probes / name
    for stringabi in (0, 1):
        name = 'cxx-' + abi + '-abi' + str(stringabi)
        files[name] = a.libgcc_probes / name
