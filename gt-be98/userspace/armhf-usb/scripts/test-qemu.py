...
root=r/'build/rootfs';image=r/'build/armhf-external-zstd157.squashfs'
...
manifest=json.loads((r/'evidence/payload.json').read_text())
...
for p in sorted((r/'usb-payload').rglob('*')):
 name='usb-image/'+p.relative_to(r/'usb-payload').as_posix();mode=p.lstat().st_mode
...
for p in runtime-armhf exception-armhf throw-armhf.so cxx-armhf-abi0 cxx-armhf-abi1; do cp /$p /newroot/tmp/probes/; done
...
run_probes() {
 EXPECTED_TRIPLET=arm-linux-gnueabihf chroot /newroot /tmp/probes/runtime-armhf
 chroot /newroot /tmp/probes/exception-armhf /tmp/probes/throw-armhf.so
 EXPECTED_TRIPLET=arm-linux-gnueabihf chroot /newroot /tmp/probes/cxx-armhf-abi0
 EXPECTED_TRIPLET=arm-linux-gnueabihf chroot /newroot /tmp/probes/cxx-armhf-abi1
}
run_probes
...
(r/'evidence/qemu-verification.json').write_text(json.dumps({'rootfs_bytes_modes_links_verified':True,'loader_checks_without_usb':len(executables),'all_armhf_tests_passed':True,'router_modified':False,'usb_filesystem_simulated_with_tmpfs':True},indent=2)+'\n')
