...
record={'verified_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
    'version':'3006.102.9-beta1-leon7','kernel':'4.19.294 #36','systemd':'257.13-gt-be98-leon7',
...
p=r/'result.json';result=json.loads(p.read_text());result.update(status='physically tested leon7; RAM accepted; firmware uncommitted',firmware_flashed=True,firmware_committed=False,physical=record);p.write_text(json.dumps(result,indent=2)+'\n')
