...
mapping = {name: 'usr/lib/systemd/' + name for name in
           ('systemd', 'systemd-executor', 'systemd-shutdown', 'systemd-journald', 'systemd-sysctl')}
mapping.update({name: 'usr/bin/' + name for name in ('systemctl', 'journalctl', 'systemd-notify', 'systemd-run', 'systemd-creds')})
