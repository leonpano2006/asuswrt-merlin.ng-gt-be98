...
base='683546d0063d32e30da34ec82e2d4a5f3d973536'
...
message='''gt-be98: record successful uncommitted leon7 hardware trial

Flash only inactive slot1 from committed slot2 and verify full readback,
unchanged fallback/bootloader, and the once-only boot selection.
Run systemd 257.13 with OpenSSL/curl/zstd/zlib/blkid on real kernel #36.

Pass all changed-file hashes, four radios, five runtime ABI probe sets,
OpenVPN, web login and rc callback, sysctl reads, TLS1.3, credentials,
zstd journal write/read and rebuilt shell/coreutils/iperf checks.
Observe 115 advancing hardware flows, 5,010 hits and 1,834,520 bytes.
Verify Docker DNS/HTTP/HTTPS, custom bridge and LAN published port;
remove all temporary test containers and networks.

Accept the trial only in RAM. Slot1 stays uncommitted and slot2 remains
the normal reboot target. Save flash/live-test source and verified receipts.
...
git('bundle','create',str(r/'systemd-features-physical-since-683546d.bundle'),branch,'^'+base)
git('bundle','verify',str(r/'systemd-features-physical-since-683546d.bundle'))
