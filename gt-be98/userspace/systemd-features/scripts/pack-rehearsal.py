...
for path in (w/'systemd-rc-next-20260917/build/probes').iterdir():shutil.copy2(path,root/'usr/libexec'/path.name)
p=root/'usr/libexec/cgroup-check.sh';p.write_text(p.read_text().replace('257.13-gt-be98)', '257.13-gt-be98-leon7)'))
for path in (r/'tests').iterdir():shutil.copy2(path,root/'usr/libexec'/path.name)
shutil.copy2(r/'build/systemd/test-sysctl-util',root/'usr/libexec/test-sysctl-util')
...
shutil.copy2(r/'build/curl-hidden-install/usr/bin/curl',root/'usr/libexec/curl-native-test')
...
branch = '''if grep -q 'lab_target=network_services' /proc/cmdline; then
