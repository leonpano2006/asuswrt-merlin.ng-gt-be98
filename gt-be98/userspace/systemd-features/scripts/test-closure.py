...
root=r/'build/production-rootfs';image=r/'build/features-final-v2.squashfs'
unpacked=r/'build/unpacked-final';assert not unpacked.exists()
...
guest=r/'build/closure-guest.cpio.gz';guest.write_bytes(gzip.compress(out,compresslevel=1,mtime=0))
# The closure runner intentionally checks the closure markers below, rather than PID1 suite markers.
subprocess.run(['python3',str(r/'scripts/run-closure-qemu.py'),'--label','closure-final','--kernel',str(w/'multiarch-loader-20260916/saved-inputs/Image36'),'--initrd',str(guest),'--complete-marker','reboot: Power down','--timeout','180'],check=True)
result=json.loads((r/'builds/qemu/closure-final/result.json').read_text())
