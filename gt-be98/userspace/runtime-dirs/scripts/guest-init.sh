...
mkdir -m 0755 /newroot/var/run
mkdir -m 0777 /newroot/var/lock
cp /runtime-probe /newroot/tmp/runtime-probe
chroot /newroot /tmp/runtime-probe
# Simulate a new boot's volatile /var without reusing old PID/socket data.
chroot /newroot /bin/busybox touch /run/runtime-reboot-sentinel
umount /newroot/var
mount -t tmpfs tmpfs /newroot/var
mkdir -m 0755 /newroot/var/run
mkdir -m 0777 /newroot/var/lock
chroot /newroot /bin/busybox test ! -e /run/runtime-reboot-sentinel
echo LAB_RUNTIME_VOLATILE_RESET_PASS
cp -a /newroot/rom/etc /newroot/tmp/etc
