...
[built-in options]
c_args = ['-g', '-frecord-gcc-switches']
c_link_args = ['-Wl,--build-id=sha1']
''')
disabled = ['initrd', 'nscd', 'utmp', 'hibernate', 'ldconfig', 'resolve', 'efi', 'tpm',
    ...
    'ima', 'idn', 'kernel-install', 'analyze', 'default-kill-user-processes']
options = ['--prefix=/usr', '--libdir=lib/aarch64-linux-gnu', '--sysconfdir=/etc', '--localstatedir=/var',
    ...
    '-Ddefault-hierarchy=legacy', '-Dsplit-usr=false', '-Dsplit-bin=true',
    '-Dversion-tag=255.22-gt-be98-lab', '-Dsysusers=true', '-Dtmpfiles=true',
...
env = dict(os.environ, PATH=str(r / 'host/bin') + ':' + os.environ['PATH'], LC_ALL='C')
