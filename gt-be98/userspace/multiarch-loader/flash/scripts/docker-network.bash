...
network=leon-ubuntu-multiarch-test
server=leon-ubuntu-multiarch-http
...
"$docker" run --rm --pull=never --memory=128m --memory-swap=192m --pids-limit=64 \
    --label leon.test=ubuntu-multiarch alpine:3.23 sh -ec '
    ip -4 addr show eth0
    ip -4 route
...
network_id=$($docker network create --label leon.test=ubuntu-multiarch "$network")
http_id=
cleanup_failed() {
    if [ -n "$http_id" ]; then
        "$docker" logs "$http_id"
...
http_id=$($docker run -d --pull=never --name "$server" --network "$network" \
    --label leon.test=ubuntu-multiarch --memory=32m --memory-swap=32m --pids-limit=32 \
    -p 192.168.100.10:18098:8080 busybox:1.37.0 sh -ec \
    'mkdir -p /www; echo GT-BE98-ubuntu-multiarch-network-ok > /www/index.html; exec busybox httpd -f -p 8080 -h /www')
...
"$docker" run --rm --pull=never --network "$network" --memory=32m --memory-swap=32m --pids-limit=32 \
    --label leon.test=ubuntu-multiarch alpine:3.23 sh -ec '
    nslookup leon-ubuntu-multiarch-http
    wget -T 20 -qO- http://leon-ubuntu-multiarch-http:8080/
    nslookup example.com
    wget -T 20 -qO /tmp/example http://example.com
    test -s /tmp/example
