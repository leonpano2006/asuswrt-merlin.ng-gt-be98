...
p = argparse.ArgumentParser(); p.add_argument('--revision', default='network-services-v5'); p.add_argument('--services-label',default='network-services-v5-services'); args = p.parse_args()
...
network = json.loads((r/'builds/qemu'/(args.revision+'-new')/'result.json').read_text())
assert network['tests_complete'] and network['guest_complete'] and not network['panic']
assert network['initramfs_sha256'] == result['initramfs_sha256']
for marker in ('LAB_NETWORK_SERVICES_ALL_PASS','LAB_NETWORK_SERVICES_PARTOF_STOP_PASS',
               'LAB_NETWORK_SERVICES_PRE_READY_CONFIG_CALLBACK_PASS',
               'LAB_NETWORK_SERVICES_RECONFIGURE_RECOVERY_ISOLATION_PASS',
               'LAB_NETWORK_SERVICES_FAILURE_NO_FALLBACK_PASS'):
    assert marker in network['lab_lines'], marker
...
image = r / 'candidate/GT-BE98_leon36-systemd257-mdns-ntpd_zstd22.pkgtb'
...
(r / 'evidence/packaging.json').write_text(json.dumps({
    'rehearsal': args.revision, 'services_rehearsal': args.services_label, 'network_rehearsal': network['label'], 'production_matches_tested_paths': True,
...
print('MDNS_NTPD_CANDIDATE_PACKED', sha(image))
