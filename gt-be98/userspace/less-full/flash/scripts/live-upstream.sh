#!/bin/busybox sh
set -eu
mkdir -p /run/leon-rmerlin-check
cd /run/leon-rmerlin-check
...
echo LIVE_UPSTREAM_EXECUTABLE_RELOCATIONS_PASS
openssl version > openssl-version
grep -q 'OpenSSL 3.5.8' openssl-version
/tmp/leon-rmerlin-openssl-legacy version > legacy-version
...
echo LIVE_UPSTREAM_VERSIONS_PASS
...
/tmp/leon-rmerlin-openssl-legacy x509 -in cert.pem -noout -subject | grep -q localhost
...
sleep 1
printf 'GET / HTTP/1.0\r\n\r\n' | /tmp/leon-rmerlin-openssl-legacy s_client -connect 127.0.0.1:14443 -CAfile cert.pem -verify_return_error > client-log 2>&1
grep -q 'Verify return code: 0 (ok)' client-log
grep -q 'TLSv1.3' client-log
kill "$tls_pid"; wait "$tls_pid" || true
trap - EXIT
echo LIVE_TLS35_SERVER_LEGACY11_CLIENT_PASS
...
test -s v2-client
echo LIVE_OPENVPN_CRYPTO_AND_TLSCRYPTV2_PASS
...
pid-file=/run/leon-rmerlin-check/dnsmasq.pid
address=/upstream.test/192.0.2.9
EOF
printf 'user=%s\ngroup=%s\n' "$(id -un)" "$(id -gn)" >> dnsmasq.conf
