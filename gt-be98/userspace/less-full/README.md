...
## Initial live installation on a3

The previous a3 firmware is read-only SquashFS. Its old less symlink points to
...
full less. The initial overlay installation required no flash or reboot.
The hook was executed twice successfully before the later full-firmware trial.
...
For manual reversal while running a3 with the overlay: first restore
...
rollback fallback are unchanged. This image has now passed the physical trial
described below; firmware commit remains separate from source Git commits.
...
## Successful physical trial

On 2026-09-17, first rebooted to the committed slot 2 / #35, then flashed this
image into inactive slot 1. Full readback matched the new rootfs hash; signed
bootfs, fallback volumes and bootloader hashes were unchanged. Slot 1 was armed
for one boot only. The router is now running leon4, kernel #36 and systemd
257.13 as PID 1. `/usr/bin/less` is native SquashFS content and no longer needs
the temporary overlay.

Passed on hardware: four radios, single systemd ownership of haveged/crond/
infosvr, actual cron execution and ASUS cron-restart notification, LAN GETINFO
discovery, authenticated management pages, HTTPD restart notification, TLS
management on port 8443, and the native less SSH PTY checks. TLS, OpenVPN and
DNS userspace regressions also passed. Docker 29.8.0 retained cgroupfs v1 and
overlay2; DNS, HTTP, certificate-verified HTTPS, custom-network DNS, LAN port
publishing and 32 MiB memory/memory+swap limits passed. Temporary test containers,
network and cron job were removed; the Docker daemon remains available.

L2/L3 hardware acceleration remains enabled. In a 20-second sample, 44 existing
hardware flows advanced 668 hits and 50,987 bytes. This AP-mode validation is
not a line-rate benchmark. No failed units or new-service restart loops were
observed; vendor Wi-Fi diagnostics remain in private logs. Physical startup
and runtime were tested; the new manager's shutdown sequence was tested in QEMU.

The successful-session marker is in RAM and the 900-second trial monitor has
exited. Firmware remains **uncommitted**: slot 1 is running, slot 2 / #35 is
still the committed normal-reboot choice, commit flags are 0/1. A normal reboot
returns to #35. See `flash/evidence/trial-result.json` for the receipt. The
hardcoded flash scripts describe this exact migration from a3; their old-image
checks must not be bypassed for a later migration.
