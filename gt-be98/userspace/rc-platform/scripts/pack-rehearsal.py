...
r=Path(__file__).resolve().parents[1]; w=r.parent; parent=w/'systemd-upgrade-20260917'
...
for path in (r/'build/probes').iterdir():
    if path.suffix != '.o': shutil.copy2(path,root/'usr/libexec'/path.name)
p=root/'usr/libexec/cgroup-check.sh';p.write_text(p.read_text().replace('257.13-gt-be98)', '257.13-gt-be98-leon7)'))
for path in (w/'systemd-features-20260917/tests').iterdir():shutil.copy2(path,root/'usr/libexec'/path.name)
shutil.copy2(w/'systemd-features-20260917/build/systemd/test-sysctl-util',root/'usr/libexec/test-sysctl-util')
from mesonbuild.scripts.depfixer import fix_rpath
from elftools.elf.elffile import ELFFile
p=root/'usr/libexec/test-sysctl-util'
with p.open('rb') as f:
 dyn=ELFFile(f).get_section_by_name('.dynamic');paths={part.encode() for tag in dyn.iter_tags() if tag.entry.d_tag in ('DT_RPATH','DT_RUNPATH') for part in getattr(tag,'runpath',getattr(tag,'rpath','')).split(':')}
fix_rpath(str(p),paths,'/usr/lib/aarch64-linux-gnu/systemd',str(p),{},False)
shutil.copy2(w/'systemd-features-20260917/build/curl-hidden-install/usr/bin/curl',root/'usr/libexec/curl-native-test')

for name in ('ssh-services-check.sh','check-ssh-daemon','check-ssh-stopped','platform-services-check.sh','platform-real-check.sh'):
    shutil.copy2(r/'tests'/name,root/'usr/libexec'/name)
...
shutil.copy2(parent/'tests/compat-check.sh',root/'usr/libexec/leon-rc-compat-check')
(root/'usr/libexec/leon-rc-compat-check').chmod(0o755)
shutil.copytree(w/'armhf-usb-20260917/usb-payload',root/'usr/libexec/leon-lab-usb-payload',symlinks=True)
...
check=check.replace('echo LAB_SYSTEMD_ALL_PASS', '/rom/etc/init.d/mount-fs.sh stop\n'
    ...
    'mkdir -p /tmp/mnt/JFFS\nmount -t tmpfs usb-simulation /tmp/mnt/JFFS\n'
    'cp -a /usr/libexec/leon-lab-usb-payload/system-libs /tmp/mnt/JFFS/\n'
    'echo LAB_USB_ARMHF_SIMULATION_READY\n'
    '/usr/libexec/leon-rc-compat-check\necho LAB_REAL_EARLY_INIT_PASS\necho LAB_SYSTEMD_ALL_PASS')
check=check.replace('echo LAB_SYSTEMD_PID1_PASS', 'echo LAB_SYSTEMD_PID1_PASS\n/usr/bin/less --version | grep -q \'^less 704 \'\n/usr/bin/less -FRSXMK /etc/os-release > /run/less-output\ncmp /etc/os-release /run/less-output\necho LAB_FULL_LESS_704_PASS')
check=check.replace('echo LAB_SYSTEMD_PID1_PASS', 'echo LAB_SYSTEMD_PID1_PASS\n/usr/libexec/features-check.sh')
check=check.replace('systemctl --no-block poweroff','leon-rcctl poweroff')
check=check.replace('echo LAB_SYSTEMD_PID1_PASS', 'echo LAB_SYSTEMD_PID1_PASS\n/usr/libexec/cgroup-check.sh')
# A separate VM runs the cron minute-boundary test; the complete prior suite
# remains unchanged in the default run.
branch = '''if grep -q 'lab_target=platform_services' /proc/cmdline; then
    /usr/libexec/platform-services-check.sh
    echo LAB_SYSTEMD_ALL_PASS
    trap - EXIT
    systemctl --no-block poweroff
    exit 0
fi
if grep -q 'lab_target=platform_real' /proc/cmdline; then
    /usr/libexec/platform-real-check.sh
    echo LAB_SYSTEMD_ALL_PASS
    trap - EXIT
    systemctl --no-block poweroff
    exit 0
fi
if grep -q 'lab_target=ssh_services' /proc/cmdline; then
    /usr/libexec/ssh-services-check.sh
    echo LAB_SYSTEMD_ALL_PASS
    trap - EXIT
    systemctl --no-block poweroff
    exit 0
fi
if grep -q 'lab_target=network_services' /proc/cmdline; then
    /usr/libexec/network-services-check.sh
    echo LAB_SYSTEMD_ALL_PASS
    trap - EXIT
    systemctl --no-block poweroff
    exit 0
fi
if grep -q 'lab_target=services' /proc/cmdline; then
    /usr/libexec/more-services-check.sh
    echo LAB_SYSTEMD_ALL_PASS
    trap - EXIT
    systemctl --no-block poweroff
    exit 0
...
'''
check=check.replace('systemctl start leon-guard.service', branch+'systemctl start leon-guard.service')
...
entries.update({'bin/busybox':(stat.S_IFREG|0o755,busybox,0,0),
    'init':(stat.S_IFREG|0o755,(parent/'scripts/rehearsal-init.sh').read_bytes(),0,0),
