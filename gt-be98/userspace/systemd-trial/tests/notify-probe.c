...
int main(int argc,char **argv)
{
    ...
    if(argc==2 && !strcmp(argv[1],"--early")) {
        assert(leon_rc_exchange(LEON_PING,0,NULL)==-1 && errno==EAGAIN);
        assert(leon_rc_exchange(LEON_SIGNAL,SIGHUP,NULL)==-1 && errno==EAGAIN);
        assert(!notify_rc("restart_httpd"));
        puts("LAB_RC_EARLY_NOTIFICATION_QUEUED_PASS");return 0;
    }
