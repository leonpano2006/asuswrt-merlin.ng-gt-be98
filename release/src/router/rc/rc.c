...
int main(int argc, char **argv)
{
	...
	*/
	...
	if (strcmp(base, "rc") == 0) {
		if (argc < 2) return 1;
		if (strcmp(argv[1], "start") == 0) return leon_rc_signal(SIGUSR2);
		if (strcmp(argv[1], "stop") == 0) return leon_rc_signal(SIGINT);
		if (strcmp(argv[1], "restart") == 0) return leon_rc_signal(SIGHUP);
		++argv;
		--argc;
		base = argv[0];
	}
...
void exe_eu_wa_rr(void){
	...
	sleep(1);
	//_dprintf("%s\n",cmd_cc_ori);
	eval("wl", "-i", ifname, "country", nvram_safe_get(ccode));
	sleep(1);	
	//_dprintf("%s\n",cmd_up);
	eval("wl", "-i", ifname, "up");
	sleep(1);
