...
export GT-BE98 := $(HND-96813_BASE)
export GT-BE98 += BUILD_NAME="GT-BE98" EXT_PHY="BCM84880" NVSIZE="192" DHDAP=y DPSTA=n REPEATER=y IPV6SUPP=y HTTPS=y ARM=y \
		...
		BCM_APPEVENTD=n LETSENCRYPT=y VPN_FUSION=y JFFS_NVRAM=y NVRAM_ENCRYPT=y NATNL_AIHOME=y BCM_CEVENTD=y UTF8_SSID=y \
		I2CTOOLS=y AMAS=y DBLOG=y ETHOBD=y CFGSYNC_LOCSYNC=y CONNDIAG=y IPSEC=STRONGSWAN IPSEC_SRVCLI_ONLY=SRV DWB=y OPEN_NAT=y \
		ROG_UI=y AMASDB=n CRASHLOG=y WATCH_REINIT=n BW160M=y BRCM_HOSTAPD=y UUPLUGIN=y IPERF3=n INFO_EXAP=y BCN_RPT=y \
		BTM_11V=y INTERNETCTRL=y ASD=y AHS=n FRS_LIVE_UPDATE=n ASUSCTRL=y HAS_5G_2=y HAS_6G_2=n CAPTCHA=y OUTFOX=y \
		GAME_MODE=y AMAS_ETHDETECT=y INSTANT_GUARD=y AVBLCHAN=y FRONTHAUL_DWB=y BCM_CLED=y BONDING=y BONDING_WAN=n \
		...
		EXTRA_KERNEL_CONFIGS="BCM_JUMBO_FRAME=y BCM_MAX_MTU_SIZE=10240 CONFIG_BCM_IGNORE_BRIDGE_MTU=y" MAX_VPNF=4 \
		WTFAST_V2=y AMAS_OB_DISABLE_DSAP=y USB_WAN_BACKUP=y AUTO_WANPORT=y \
		NFS=n SNMPD=y TOR=y UPNPIGD2=y NANO=n NTPD=y DNSPRIVACY=y IPSET=y VPN_FUSION_MERLIN=y
		NFS=n SNMPD=y TOR=y UPNPIGD2=y NANO=n NTPD=y IPSET=y VPN_FUSION_MERLIN=y
